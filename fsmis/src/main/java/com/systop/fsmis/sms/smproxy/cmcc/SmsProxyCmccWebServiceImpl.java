...
public class SmsProxyCmccWebServiceImpl implements SmsProxy {
  ...
  public void querySmsSendState(SmsSend smsSend) throws Exception {
    ...
    String[] masState = service.querySmReport(SmsConstants.CONN_NAME,
        SmsConstants.CONN_PASS, smsSend.getMasid());
    if (!ArrayUtils.isEmpty(masState) && masState.length == 4) {
      if ("0".equals(masState[3])) {
        smsSend.setIsReceive(SmsConstants.SMS_SEND_IS_RECEIVE_Y);
      } else {
        smsSend.setIsReceive(SmsConstants.SMS_SEND_IS_RECEIVE_N);
      }
    } else {
      smsSend.setIsReceive(SmsConstants.SMS_SEND_IS_RECEIVE_UNKNOWN);
    }
  }
