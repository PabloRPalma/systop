...
<div class="x-panel">
<div class="x-panel-header">协调指挥&nbsp;&gt;&nbsp;${param['isMultiple'] eq 0?'一般任务':'综合任务'}接收&nbsp;&gt;&nbsp;${param['isMultiple'] eq 0?'一般任务':'综合任务'}任务列表</div>
<div class="x-toolbar">
<table width="99%">
	<tr>
	...
	<ec:row>
		...
		<ec:column width="60" property="_0" title="查看"
			...
			<c:if test="${item.status == '0' or item.status == '1'}">
				<a href="${ctx}/taskdetail/receiveTask.do?model.id=${item.id}&isMultiple=${param['isMultiple']}&modelId=${param['modelId']}">接收|</a>
			...
			<c:if test="${item.status == '2'}">
				<a href="${ctx}/taskdetail/toDealWithTaskDetail.do?model.id=${item.id}&isMultiple=${param['isMultiple']}&modelId=${param['modelId']}">处理|</a>
