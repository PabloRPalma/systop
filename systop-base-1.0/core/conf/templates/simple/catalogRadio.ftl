<#if data.catalogs?exists>
  <#list data.catalogs as c>   
       <input type="radio" id="${data.name}" name="${data.name}"<#if data.onclick?exists> 
...
<#if c.getSelected()==true || (data.defaultValue?exists && data.defaultValue==c.getValue())> checked 
</#if>>${c.getLabel()}&nbsp;
