...
import com.systop.cms.webview.service.MetadataFreeMarkerManager;
...
public class CmsWebViewServlet extends BaseFreeMarkerServlet {
  ...
  protected StringTemplateContext reqUrlToModelCtx(HttpServletRequest req,
    ...
    if (stc != null) {
      ...
      stc.getModel().put(CmsConstants.METADATA_BENA, getMetadataFmMgr());
      // 设置request对象
      stc.getModel().put("req", req);
      // 网站访问根目录
      stc.getModel().put("ctx", req.getContextPath());
      // 网站基本信息
      stc.getModel().put("siteCfg", datashare.admin.sitecfg.SiteConstants.siteCfg);
    }
    return stc;
  }
  ...
  /** 通过Spring获得MetadataFreeMarkerManager */
  private MetadataFreeMarkerManager getMetadataFmMgr() {
    return (MetadataFreeMarkerManager) getBean("metadataFreeMarkerManager");
  }
}
