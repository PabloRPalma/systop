...
public class SmsSendManager extends BaseGenericsManager<SmsSend> {
  ...
  /**
   * 得到已经发送到Mas并且未知接收状态的短信方法
   * 
   * @return
   */
  public List<SmsSend> getSendedSmsSends() {
    StringBuffer buf = new StringBuffer("from SmsSend ss where ss.isNew = ");
    buf.append(SmsConstants.SMS_SMS_SEND_IS_NOT_NEW);
    buf.append(" and ss.isReceive is null");   
    buf.append(" and ss.masid is not null");
