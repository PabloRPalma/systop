...
var ECSide = function(formid) {
	...
	Me.buildPrepArea = function() {
		...
		var hasPrepareaNext = document
				.getElementById(this.prepareaName['next']);
		if (!hasPrepareaNext) {
		...
		var hasPrepareaPrev = document
				.getElementById(this.prepareaName['prev']);
		if (!hasPrepareaPrev) {
	...
	Me.goPage = function() {
		...
		if (newPageNO == Me.prepPage['next'] && Me.prepState['next'] == 2) {
			key = 'next';
				...
				&& Me.prepState['prev'] == 2 && Me.doPreloadPrev) {
			key = 'prev';
		}
	...
	Me.dealResponse = {
		'next' : function(originalRequest) {
			$_E(Me.prepareaName['next']).value = ECSideUtil.cutText(
					originalRequest.responseText, Me.id);
			Me.prepState['next'] = 2;
			Me.doingAjaxSubmit = false;

		},
		'prev' : function(originalRequest) {
			$_E(Me.prepareaName['prev']).value = ECSideUtil.cutText(
					originalRequest.responseText, Me.id);
			Me.prepState['prev'] = 2;
			Me.doingAjaxSubmit = false;
		}
	...
	Me.ajaxPrep = function(which) {
		...
		if (Me.prepPage[key] < 1
				|| Me.prepPage[key] > ($_E(Me.totalPagesFieldName).value / 1)) {
			return;
		}
	...
	Me.ajaxSubmit = function(resfunc, asy, parameter) {
        ...
        jQuery.ajax({
            ...
            async: true, //It must be true, or the waiting bar can not show in IE
            complete: resfunc
        });
	};
	...
	Me.init = function() {
		...
		Me.doPreloadPrev = Me.doPrepPrev == "temp"
				? Me.doPreloadPrev
				: Me.doPrepPrev;
		...
		if (window.isIE) {
			var hideHeader = document.getElementById(Me.id
					+ ECSideConstants.HIDE_HEADER_ROW);
			if (hideHeader) {
		...
		if (window.frameElement
				&& window.frameElement.name == Me.EXPORT_IFRAME_ID) {
		...
		if (Me.sortedColumnHearderId) {
			var sortedHeader = document
					.getElementById(Me.sortedColumnHearderId);
			if (sortedHeader && Me.sortType && Me.sortType != ''
					&& Me.sortType != 'default') {
				var newHtml = ECSideUtil.trimString(sortedHeader.innerHTML, -1)
						+ "&#160;<div class=\"sort" + Me.sortType.toUpperCase()
						+ "\"></div>";
		...
		Me.ECListHeadZone = document.getElementById(Me.id
				+ ECSideConstants.HEAD_ZONE_SUFFIX);
		Me.ECListBodyZone = document.getElementById(Me.id
				+ ECSideConstants.BODY_ZONE_SUFFIX);
		...
		Me.ECListToolbarTable = document
				.getElementById(Me.id + "_toolbarTable");
		Me.ECListToolbarShadow = document.getElementById(Me.id
				+ "_toolbarShadow");

		if (Me.ECListToolbarTable) {
			Me.ECListToolbarShadow.style.height = Me.ECListToolbarTable.offsetHeight
					+ 2 + "px";
		}
	...
	Me.initWaitingBar = function() {
        ...
        //Place the waiting bar at the center of the document.
		var wl = ECSideUtil.getPosLeft(Me.ECForm);
		var wr = ECSideUtil.getPosRight(Me.ECForm);
		var wt = ECSideUtil.getPosTop(Me.ECForm);
		var wh = jQuery(Me.ECForm).height();
		var wbW = jQuery('#'+ Me.id + "_waitingBarCore").width();
		var wbH = jQuery('#'+ Me.id + "_waitingBarCore").height();
		Me.waitingBar.style.left = (wl + (wr - wl)/2 - wbW/2) + 'px';
		Me.waitingBar.style.top = (wh/2 - wbH) + "px";
		if(jQuery.browser.msie ) {
			Me.waitingBarCore.style.height = '30px';
			Me.waitingBarCore.style.paddingTop = '8px';
		}
	};
	...
	Me.resizeWaitinBar = function() {
		if (Me.waitingBar && Me.waitingBar.getAttribute("big") == "true") {
			...
			Me.waitingBarCore.style.left = parseInt(Me.waitingBar.style.left)
					+ (w - cw - 50) / 2 + "px";
			Me.waitingBarCore.style.top = parseInt(Me.waitingBar.style.top)
					+ (h - ch - 40) / 2 + "px";
		}
	};
	...
	Me.showWaitingBar = function() {
		...
		Me.waitingBarCore.style.zIndex = 1000;
	};
	...
	Me.autoFitHeight = function() {
		...
		if (Me.ECListBodyZone.offsetHeight >= Me.ECListBody.parentNode.scrollHeight) {
			var dh = Me.ECListBodyZone.offsetHeight
					- Me.ECListBodyZone.clientHeight
					+ ECSideConstants.LIST_HEIGHT_FIXED;
			if (dh <= 2
					&& Me.ECListBodyZone.offsetWidth
							- Me.ECListBodyZone.clientWidth > 2) {
	...
	Me.resizeHeader = function() {
		...
		Me.ECListHeadZone.style.width = Me.ECListBodyZone.clientWidth
				+ fixWidth + "px";
	};
	...
	Me.initList = function() {
		...
		if (!Me.isClassic) {
			...
			if (window.isIE) {
				Me.ECListHead.parentNode.style.tableLayout = "fixed";
				ECSideUtil_addEvent(Me.ECListBodyZone, "resize",
						Me.handleResize);
			} else {
				// Me.ECListHead.parentNode.style.tableLayout="fixed";
				// Me.ECListBody.parentNode.style.tableLayout="fixed";
			...
			Me.orgListHeight = ECSideUtil
					.parseIntOrZero(Me.ECListBodyZone.style.height);
		}
	};
	...
	if (rows1 && rows2 && rows1.length > 0 && rows2.length > 0) {
		...
		if (window.isIE) {
			...
			for (var i = 0;i < colNum; i++) {
				...
				var brw = 0;
				brw = headTDs[i].getAttribute("resizeColWidth") == "true"
						? 2
						: (i == 0 ? 0 : (i + 1 == colNum ? 2 : 0));
				...
				if (headTDs[i].getAttribute("resizeColWidth") != "true"
						&& dw != 0) {
...
ECSideUtil.getMessage = function(name, msgs) {
	...
	for (var i = 1;i < arguments.length; i++) {
		msgTemplate = ECSideUtil.replaceAll(msgTemplate, "#{" + i + "}",
				arguments[i]);
	}
	return msgTemplate;
};
...
ECSideUtil.cutText = function(text, formid) {
	...
	var begin = ECSideConstants.AJAX_ZONE_PREFIX
			+ ECSideConstants.AJAX_ZONE_BEGIN + formid
			+ ECSideConstants.AJAX_ZONE_SUFFIX;
	var end = ECSideConstants.AJAX_ZONE_PREFIX + ECSideConstants.AJAX_ZONE_END
			+ formid + ECSideConstants.AJAX_ZONE_SUFFIX;
...
ECSideUtil.reload = function(formid, pageno) {
	...
	ECSideUtil.showShadowRow(ecsideObj.currentShadowRowParent,
			ecsideObj.currentShadowEventSrc, formid);
};
...
ECSideUtil.doSort = function(event, columnAlias, sortT1, formid,
		columnHearderId) {
...
ECSideUtil.doExport = function(type, fileName, page, formid) {
	...
	if (page === false && maxRowsExported
			&& ECSideUtil.parseIntOrZero(maxRowsExported) > 0) {
		if (ECSideUtil.parseIntOrZero(maxRowsExported) < ECSideUtil
				.getTotalRows(formid)) {
			alert(ECSideUtil.getMessage("OVER_MAXEXPORT", ECSideUtil
					.parseIntOrZero(maxRowsExported)));
			return;
		}
	}
...
ECSideUtil.selectRow = function(rowObj, formid) {
	...
	var ecsideObj = ECSideUtil.getGridObj(formid);
	if (!ecsideObj || rowObj == ecsideObj.selectedRow) {
		return;
	}
...
ECSideUtil.unlightHeader = function(tdObj, formid) {
	...
	if (className) {
		className = className.split(" ");
		if (className[0].lastIndexOf("Over") == className[0].length
				- "Over".length) {
			className[0] = className[0].substring(0, className[0].length
					- "Over".length);
		}
	}
	tdObj.className = className.join(" ");
...
ECSideUtil.removeClass = function(object, className) {
	...
	object.className = object.className.replace(new RegExp('(^|\\s)'
			+ className + '(\\s|$)'), ' ');
};

ECSideUtil.addClass = function(object, className) {
	if (!object || ECSideUtil.hasClass(object, className)) {
		return;
	}
	if (object.className) {
...
ECSideUtil.NearPagesBar.createNearPagesList = function(nearPages, formid) {
	...
	for (pi1 = startP;pi1 <= endP; pi1++) {
		if (pi1 == page) {
			...
			listHtml += "<a href=\"#\" onclick=\"ECSideUtil.gotoPage(" + pi1
					+ ",'" + formid + "');return false;\" >" + pi1 + "</a>";
		}
	}
	listHtml += "</nobr>";
	return listHtml;
...
ECSideUtil.NearPagesBar.showMe = function(fireObj, formid) {
	...
	var nearPages = ECSideUtil.parseIntOrZero(ecsideObj.ECForm
			.getAttribute("nearPages"));
	if (nearPages < 2) {
		return;
	}
	var page = ecsideObj.getPageNo();
	...
	this.coreElement.style.left = ECSideUtil.getPosLeft(fireObj) - dx
			+ document.body.scrollLeft + "px";
	this.coreElement.style.top = (ECSideUtil.getPosTop(fireObj)
			- this.coreElement.offsetHeight + document.body.scrollTop)
			+ "px";
};
...
ECSideUtil.doFilter = function(event, filterInputObj, filterFieldName, formid) {
	...
	ECSideUtil.doFilterCore(filterInputObj, filterFieldName,
			ECSideConstants.FILTER_ACTION, formid);
};
ECSideUtil.doClearFilter = function(event, filterInputObj, filterFieldName,
		formid) {
	ECSideUtil.doFilterCore(filterInputObj, filterFieldName,
			ECSideConstants.CLEAR_ACTION, formid);
};

ECSideUtil.doFilterCore = function(filterInputObj, filterFieldName, faction,
		formid) {
...
ECSideUtil.ColmunMenu.getFilterItem = function(menuWidth, columnObj,
		columnName, formid) {
	var FIX_WIDTH = 20;
	...
	var jsfunction = "ECSideUtil.doFilter(event,this,'" + filterFieldName
			+ "','" + formid + "')";
	// this.coreElement.style.width=width+"px";
	var filterHTML = "<nobr><input type=\"button\" class=\"filterIcon\" onclick=\""
			+ jsfunction + "\" />";
	...
	if (templateText.toLowerCase().indexOf("<select ") == 0) {
		...
		templateText = ECSideUtil.replaceAll(templateText, " name=\"",
				" tempname=\"");
		templateText = ECSideUtil.replaceAll(templateText, " id=\"",
				" tempid=\"");
		templateText = ECSideUtil.replaceAll(templateText, " style=\"",
				" tempstyle=\"");
		templateText = ECSideUtil.replaceAll(templateText, " class=\"",
				" tempclass=\"");
		templateText = ECSideUtil.replaceAll(templateText, " value=\"" + v
				+ "\"", " value=\"" + v + "\" selected=\"selected\" ");

		filterHTML += "<select onclick=\"ECSideUtil_stopEvent(event)\" style=\"width:"
				+ (menuWidth - FIX_WIDTH)
				+ "px\" filterfield=\"true\" onkeypress=\""
				+ jsfunction
				+ "\" " + templateText;
	} else {
		filterHTML += "<input type=\"text\" class=\"filterInput\" value=\""
				+ v
				+ "\" style=\"width:"
				+ (menuWidth - FIX_WIDTH)
				+ "px\" onclick=\"ECSideUtil.doFilterFocus(event,this)\" onkeypress=\""
				+ jsfunction + "\" /></nobr>";
	}
	var clearHTML = "<nobr><input type=\"button\" class=\"clearIcon\" />";
	clearHTML += "<span class=\"itemText\" style=\"width:"
			+ (menuWidth - FIX_WIDTH)
			+ "px\" onclick=\"ECSideUtil.doClearFilter(event,this,'"
			+ filterFieldName + "','" + formid + "')\" >"
			+ ECSideMessage.FILTERCLEAR_TEXT + "</span></nobr>";

	return filterHTML + "<br />" + clearHTML;

};

ECSideUtil.ColmunMenu.getSortItem = function(menuWidth, columnObj, columnName,
		formid) {
	...
	var jsfunctionAsc = "ECSideUtil.doSort(event,'" + columnName + "','asc','"
			+ formid + "')";
	var jsfunctionDesc = "ECSideUtil.doSort(event,'" + columnName
			+ "','desc','" + formid + "')";
	var jsfunctionDefault = "ECSideUtil.doSort(event,'" + columnName
			+ "','default','" + formid + "')";

	var ascHTML = "<nobr><input type=\"button\" class=\"ascIcon\" />";
	ascHTML += "<span class=\"itemText\" style=\"width:"
			+ (menuWidth - FIX_WIDTH) + "px\" onclick=\"" + jsfunctionAsc
			+ "\" >" + ECSideMessage.SORTASC_TEXT + "</span></nobr>";

	var descHTML = "<nobr><input type=\"button\" class=\"descIcon\" />";
	descHTML += "<span class=\"itemText\" style=\"width:"
			+ (menuWidth - FIX_WIDTH) + "px\" onclick=\"" + jsfunctionDesc
			+ "\" >" + ECSideMessage.SORTDESC_TEXT + "</span></nobr>";

	var defaultHTML = "<nobr><input type=\"button\" class=\"defaultIcon\" />";
	defaultHTML += "<span class=\"itemText\" style=\"width:"
			+ (menuWidth - FIX_WIDTH) + "px\" onclick=\"" + jsfunctionDefault
			+ "\" >" + ECSideMessage.SORTDEFAULT_TEXT + "</span></nobr>";
...
ECSideUtil.ColmunMenu.showMe = function(fireObj, formid) {
	...
	var filterItemHTML = this.getFilterItem(menuWidth, fireObj, columnName,
			formid);
	...
	this.coreElement.style.top = ECSideUtil.getPosBottom(fireObj)
			+ document.body.scrollTop + "px";
	this.coreElement.style.display = "block";
	var tempLeft = ECSideUtil.getPosLeft(fireObj) + document.body.scrollLeft;
	if (tempLeft + this.coreElement.offsetWidth >= document.body.clientWidth
			- 2) {
...
ECSideUtil.StartResize = function(event, obj, formid) {
	...
	ECSideUtil.leftC = ECSideUtil.getPosLeft(ECSideUtil.Dragobj)
			+ parseInt(ECSideUtil.MinColWidth);
	if (way == "resizeTable" || !ECSideUtil.DragobjNext) {
		if (ECSideUtil.DragECSideObj.ECListBodyZone) {
			ECSideUtil.rightC = ECSideUtil
					.getPosRight(ECSideUtil.DragECSideObj.ECListBodyZone);
		} else {
			ECSideUtil.rightC = document.body.clientWidth;
		}

	} else {
		ECSideUtil.rightC = ECSideUtil.getPosRight(ECSideUtil.DragobjNext)
				- parseInt(ECSideUtil.MinColWidth);
	}
	...
	try {
		ECSideUtil.DragobjBodyCellNext = ECSideUtil
	...
	if (!ECSideUtil.MinColWidth || ECSideUtil.MinColWidth == ''
			|| ECSideUtil.MinColWidth < 1) {
		ECSideUtil.MinColWidth = ECSideConstants.MIN_COLWIDTH;
	}

	ECSideUtil.separateLine.style.top = ECSideUtil
			.getPosTop(ECSideUtil.DragECSideObj.ECListHead)
			+ 2;
	...
	// ECSideUtil.startC=ECSideUtil.getPosRight( ECSideUtil.Dragobj
	// )-ECSideUtil.parseIntOrZero(ECSideUtil.separateLine.style.width)+document.body.scrollLeft-ECSideUtil.DragECSideObj.ECListBodyZone.scrollLeft;
...
ECSideUtil.EndResize = function(event) {
	...
	ECSideUtil.endC = ECSideUtil
			.parseIntOrZero(ECSideUtil.separateLine.style.left);
	...
	/*
	 * if (ECSideUtil.DragobjNext &&
	 * ECSideUtil.DragECSideObj.resizeWay!="resizeTable"){
	 * cc=ECSideUtil.DragobjBodyCellNext.clientWidth+dWidth+fixX; if (cc<10){
	 * cc=10; } ECSideUtil.DragobjNext.style.width=cc+"px";
	...
	if (!ECSideUtil.DragECSideObj.isClassic) {
		ECSideUtil.syncRowsWidth(ECSideUtil.DragECSideObj.ECListHead.rows,
				ECSideUtil.DragECSideObj.ECListBody.rows);
	}
...
ECSideUtil.resizeInit = function() {
	...
	/*
	 * var e = event||window.event; e.cancelBubble = true e.returnValue = false;
	 * return false;
	 */
};
...
ECSideUtil.updateCell = function(cellObj) {
	//By SAM,2008-11-12
	var elems = jQuery(cellObj).children();
	
	if (elems.length > 0) {
		ECSideUtil.updateCellContent(cellObj, elems[0]);
	}
};
...
ECSideUtil.updateCellContent = function(cellObj, elementObj) {
	...
	if (editType == "input") {
		...
		if(elementObj.options.length > 0) {//By SAM,2008-11-12
			var selOpt = elementObj.options[elementObj.selectedIndex];			
		   value = selOpt.value;
		   cellObj.innerHTML = selOpt.text;
		} else {
			cellObj.innerHTML = "&nbsp;";
		}
	} else if (editType == "checkbox" || editType == "radio") {
...
ECSideUtil.getRemoveUpdatedClassRows = function(listRows, recordKey) {
	...
	if (listRows && listRows.length) {
		for (var i = 0;i < listRows.length; i++) {
			if (listRows[i].getAttribute("recordKey") == ECSideUtil
					.trimString(recordKey)) {
...
ECSideUtil.getRemoveDeletedRows = function(listRows, recordKey) {
	...
	if (listRows && listRows.length) {
		for (var i = 0;i < listRows.length; i++) {
			var crow = listRows[i];
			if (crow
					&& crow.getAttribute("recordKey") == ECSideUtil
							.trimString(recordKey)
					&& ECSideUtil.hasClass(crow, "del")) {
				var crowIndex = crow.rowIndex;
				if (crow.getAttribute("hasShadow") == "true") {
					crow.parentNode.removeChild(crow.parentNode.rows[crowIndex
							+ 1]);
				}
...
ECSideUtil.updateShadowRow = function(crow, shadowRow, formid, originalRequest) {
	...
	if (ECSideUtil.trimString(text) != '') {
		...
		shadowRow.cells[0].innerHTML = ECSideMessage.SHADOWROW_FAILED
				+ " ( recordKey : " + crow.getAttribute("recordKey") + ").";
	}

	var shadowRowHeight = ECSideUtil
			.parseIntOrZero(shadowRow.cells[0].scrollHeight);
	shadowRow.setAttribute("shadowRowHeight", shadowRowHeight);
	shadowRow.cells[0].style.height = shadowRowHeight + "px";

	var shadowRowHeight = ECSideUtil.parseIntOrZero(shadowRow
			.getAttribute("shadowRowHeight"));
	var ecsideObj = ECSideUtil.getGridObj(formid);
	ecsideObj.hideWaitingBar();
...
ECSideUtil.showShadowRow = function(crow, eventSrc, formid) {
	...
	if (isShowed && ecsideObj.autoCloseOtherShadowRow) {
		if (ecsideObj.currentShadowRowParent && ecsideObj.currentShadowEventSrc
				&& crow.id != ecsideObj.currentShadowRowParent) {
			ECSideUtil.showShadowRow(ecsideObj.currentShadowRowParent,
					ecsideObj.currentShadowEventSrc, ecsideObj.id);
		}
	}
	if (isShowed) {
...
ECSideUtil.saveGirdBatch = function(buttonObj, formid, isBatch) {
	...
	for (var i = 0;i < rows.length; i++) {
		var pars = ECSideUtil
				.getRowCellsMap(ecsideObj.forUpdateRows[i], formid);
		if (doBatch) {
			ECSideUtil.appendMap(parsMap, pars);
		} else {
			ECSideUtil.doAjaxUpdate(urlu, pars, ecsideObj.updateCallBack,
					formid);
		}
	...
	if (doBatch && rows.length > 0) {
		ECSideUtil
				.doAjaxUpdate(urlu, parsMap, ecsideObj.updateCallBack, formid);
	}
	...
	for (var i = 0;i < rowsi.length; i++) {
		//Must call clone to serialize,or Ecside can't find the inserted TR element.
		//By SAM 2008-11-13
		var v = jQuery("<form></form>").append(jQuery(ecsideObj.forInsertRows[i]).clone());
		var pars = v.serialize();
		
		if (doBatch) {
			ECSideUtil.appendMap(parsMap, pars);
		} else {
			ECSideUtil.doAjaxUpdate(urli, pars, ecsideObj.insertCallBack,
					formid);
		}
	...
	if (doBatch && rowsi.length > 0) {
		ECSideUtil
				.doAjaxUpdate(urli, parsMap, ecsideObj.insertCallBack, formid);
	}
	...
	for (var i = 0;i < rowsd.length; i++) {
		var pars = ECSideUtil
				.getRowCellsMap(ecsideObj.forDeleteRows[i], formid);
		if (doBatch) {
			ECSideUtil.appendMap(parsMap, pars);
		} else {
			ECSideUtil.doAjaxUpdate(urld, pars, ecsideObj.deleteCallBack,
					formid);
		}
	}
	if (doBatch && rowsd.length > 0) {
		ECSideUtil
				.doAjaxUpdate(urld, parsMap, ecsideObj.deleteCallBack, formid);
	}
...
ECSideUtil.addToGird = function(buttonObj, templateId, formid) {
	...
	if (!template) {
		template = document
				.getElementById(ECSideConstants.DEFALUT_ADD_TEMPLATE);
	}
	...
	ECSideUtil_addEvent(newTr, "click", ECSideUtil.selectRow.bind(this, newTr,
			ecsideObj.id));
...
ECSideUtil.doAjaxUpdate = function(url, pars, callBack, formid) {
	...
	//Using jQuery as ajax engine. By SAM.
	jQuery.ajax({
        	type: 'POST',
            url: url,
            data: pars,
...
ECSideUtil.changeListHeight = function(height, formid) {
	...
	if ("auto" != height) {
		if (height.indexOf('+') == 0) {
			height = ecsideObj.listHeight
					+ ECSideUtil.parseIntOrZero(height.substring(1));
	...
	if (ECSideUtil.parseIntOrZero(height) > ecsideObj.ECListBody.scrollHeight
			- ECSideConstants.OFFSET_A
			|| height == "auto") {
		/* divSYT.style.overflowY="hidden"; */
		height = ecsideObj.ECListBody.parentNode.scrollHeight;
		var dh = ecsideObj.ECListBodyZone.offsetHeight
				- ecsideObj.ECListBodyZone.clientHeight
				+ ECSideConstants.LIST_HEIGHT_FIXED;
		if (dh <= 2
				&& Me.ECListBodyZone.offsetWidth
						- Me.ECListBodyZone.clientWidth > 2) {
	...
	height = height < ecsideObj.minHeight / 1
			? ecsideObj.minHeight / 1
			: height;
	if (ecsideObj.ECListBodyZone) {
...
/*
 * (function(){ var gird=ECSideUtil.createGird(''); gird.useAjax=true;
 * gird.doPre })();
 */
//By Sam,Use My97 as the datetime picker
ECSideUtil.showCalendar = function(inputObj) {
	$(inputObj).click(function() {
		WdatePicker({skin:'whyGreen', 
			onpicked:function() {
			    ECSideUtil.fillDate(inputObj);
		    }
		});
	});
};
//By Sam
ECSideUtil.fillDate = function(inputField) {
	ECSideUtil.updateEditCell(inputField);
	$(inputField).hide();
};
//copy from prototype.js v1.5 By SAM 
...
var ECCN = function(formid) {
	/*
	 * 
	 * eccn.ajaxSubmit=false; eccn.doPrep=false; eccn.doPrepPrev=false;
	 * 
	 */
...
/**
 * User can control the ecgrid out of the ECSide by this function.
 * @param options {
 *    url: 'no need to explain.',
 *    ecId: 'ec grid id',
 *    queryFormId:'the form that contains the query params',
 *    minHeight: 'the same as the ecside taglib's attribute'
 * }
 */
var query = function(options) {
    jQuery.ajax({
	    url: options.url,
	    data: jQuery('#' + options.queryFormId).serialize(),
	    beforeSend: function() {
	       var wb = jQuery('#' + options.ecId + '_waitingBarCore');
	       wb.css('left', jQuery(document).width()/2 - wb.width()/2 + 'px');
	       wb.css('top', jQuery(document).height()/2 - wb.height()/2 + 'px');
	       wb.show();
	    },
	    complete : function(xhr){
	        var gird=ECSideUtil.createGird(options.ecId); 
			 gird.useAjax=true;  
			 gird.doPreload=false; 
			 gird.isClassic=false; 
			 gird.minHeight=options.minHeight; 
			 gird.fillForm(xhr);
			 gird = null;
	    }
    });
