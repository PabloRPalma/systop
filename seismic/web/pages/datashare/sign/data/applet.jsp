<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="java.util.List,quake.sign.data.model.Criteria"%>
...
<%
	...
	for (Criteria c : criteriaList) {
		comId = c.getStationId() + "_" + c.getPointId() + "_"
				+ c.getItemId();
%>
<applet id="<%=comId%>" code="quake.sign.data.webapp.SignApplet"
...
<div style="width: 800px" align="right">
	<a href="${ctx}/pages/quake/sign/data/security/readme.jsp">
		<img style="width:18px;height:18px" src="${ctx}/images/exticons/icon-question.gif">&nbsp;无法保存曲线图
	</a>
	<a href="${ctx}/quake/sign/log/list.do?model.stationId=<%=c.getStationId() %>&model.pointId=<%=c.getPointId() %>&model.itemId=<%=c.getItemId() %>&model.tableCategory=DLG&model.startDate=<%=request.getAttribute("start")%>&model.endDate=<%=request.getAttribute("end")%>">
