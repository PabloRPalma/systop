...
<script type="text/javascript">
	$(function() {
		...
		$('#meta').click(function() {
			...
			$.ajax({
				url: '${ctx}/quake/metadata/getMetadataInfo.do',
				type: 'post',
				dataType: 'json',
				data: {type : mType},
		...
		$('#delmeta').click(function() {
			var mType = $("#type")[0].value;
			$.ajax({
				url: '${ctx}/quake/metadata/deleteMetadata.do',
				type: 'post',
				dataType: 'json',
				data: {type : mType},
				success: function(result, textStatus){
		...
		$("#dlg").dialog({
			...
			buttons: {
				'保存': function() {
					...
					var accUrl = '${ctx}/quake/descr/queryDescribe.do';
					if (textTitle == null || textTitle == '') {
						alert("请输入标题！");
						return;
					}
					$.ajax({
						url: '${ctx}/quake/descr/save.do',
						type: 'post',
						dataType: 'json',
		...
		$('#memo').click(function() {
			...
			var accUrl = '${ctx}/quake/descr/queryDescribe.do';
			$.ajax({
				url: '${ctx}/quake/descr/getDescrInfo.do',
				type: 'post',
				dataType: 'json',
		...
		$('#queryMeta').click(function() {
			...
			window.location = '${ctx}/quake/metadata/queryMetadataInfo.do?model.type='+mType;
		});
		$('#downMeta').click(function() {
			var mType = $("#mType")[0].value;
			window.location = '${ctx}/quake/metadata/downloadMetadataInfo.do?model.type='+mType;
		});
...
<div id="dialog" title="元数据管理">
	<form action="${ctx}/quake/metadata/save.do" method="POST" 
